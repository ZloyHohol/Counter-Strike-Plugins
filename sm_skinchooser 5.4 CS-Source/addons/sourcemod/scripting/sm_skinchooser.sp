...
ConVar g_enabled;
ConVar g_steamid;
ConVar g_arms_enabled;
ConVar g_autodisplay;
ConVar g_displaytimer;
ConVar g_AdminGroup;
ConVar g_AdminOnly;
ConVar g_SkinBots;
ConVar g_SkinAdmin;
ConVar g_SkinAdminTimer;
ConVar g_SkinAdminTimerEnabled;
ConVar g_ForcePlayerSkin;
ConVar g_ForcePlayerSkinTimer;
ConVar g_ForcePlayerSkinTimerEnabled;
ConVar g_CommandCountsEnabled;
ConVar g_CloseMenuTimer;
ConVar g_menustarttime;
ConVar g_CommandCounts;
ConVar g_mapbased;
ConVar g_blockAgentModels;
ConVar g_EnableThirdPersonMessage;
...
public int LoadModels(char[][] models, char[] ini_file)
{
...
{	
	...
	{	
		GetClientAuthId(client,AuthId_Steam2, authid[client], sizeof(authid[client]));
	}
	else if(GetConVarInt(g_steamid) == 1)	
	{	
		GetClientAuthId(client,AuthId_Steam3, authid[client], sizeof(authid[client]));
	}	
...
{
	if (playermodelskv == INVALID_HANDLE)
	{
		return;
	}
	// Get the user's authid			
...
public Action skin_players(Handle timer, any client)
{
	...
	if (team==2)
	{
		if (g_ModelsPlayer_Count_Team2 > 0)
		{
			SetEntityModel(client,g_ModelsPlayerTeam2[GetRandomInt(0, g_ModelsPlayer_Count_Team2-1)]);
		}
	}
	...
	{
		if (g_ModelsPlayer_Count_Team3 > 0)
		{
			SetEntityModel(client,g_ModelsPlayerTeam3[GetRandomInt(0, g_ModelsPlayer_Count_Team3-1)]); 
		}
	}
...
public Action skin_admin(Handle timer, any client)
{
	...
	if (team==2)
	{
		if (g_ModelsAdmin_Count_Team2 > 0)
		{
			SetEntityModel(client,g_ModelsAdminTeam2[GetRandomInt(0, g_ModelsAdmin_Count_Team2-1)]);
		}
	}
	...
	{
		if (g_ModelsAdmin_Count_Team3 > 0)
		{
			SetEntityModel(client,g_ModelsAdminTeam3[GetRandomInt(0, g_ModelsAdmin_Count_Team3-1)]);		
		}
	}
	...
	{			
		...
		{			
			...
			for (int a = 0; a < sizeof(anarchistModelsT); a++) 
			{
    			...
    			{
  			 		SetEntityModel(client,pirateModelsT[GetRandomInt(0 , sizeof(pirateModelsT)-1)]);  					
  				}  					
			} 
			...
			for (int c = 0; c < sizeof(balkanModelsT); c++) 
			{
    			...
    			{
   			 		SetEntityModel(client,anarchistModelsT[GetRandomInt(0 , sizeof(anarchistModelsT)-1)]); 					
  				}  					
			}	
			...
			{			
				for (int h = 0; h < sizeof(AgentModelsT); h++) 
				{
    				...
    				{
   				 		SetEntityModel(client,phoenixModelsT[GetRandomInt(0 , sizeof(phoenixModelsT)-1)]);
		...
		{			
			...
			for (int i = 0; i < sizeof(fbiModelsCT); i++) 
			{
    			...
    			{
   			 		SetEntityModel(client,gignModelsCT[GetRandomInt(0 , sizeof(gignModelsCT)-1)]);  					
				}
			}		
			...
			{			
    			...
    			{
					SetEntityModel(client,fbiModelsCT[GetRandomInt(0 , sizeof(fbiModelsCT)-1)]);						
				}
			}	
			...
			{			
				for (int o = 0; o < sizeof(AgentModelsCT); o++) 
				{
    				...
    				{
   				 		SetEntityModel(client,sasModelsCT[GetRandomInt(0 , sizeof(sasModelsCT)-1)]);
						PrintToChat(client, "Sorry using of AgentModels is not allowed on this Server!!!");
   				 	}   				 		
				}
			}
