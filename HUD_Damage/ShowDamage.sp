...
#define PROJECT_FULLNAME	"ShowDamage"
...
public void OnClientCookiesCached(int iClient)
{
	...
	if(szValue[0])
	{
		StyleShowDamage[iClient] = view_as<char>(szValue);
	}
	else
	{
		StyleShowDamage[iClient] = "NewStyle";
	}
}
...
{
	int iAttacker = GetClientOfUserId(hEvent.GetInt("attacker")), iDamage = hEvent.GetInt("dmg_health"), iVictim = GetClientOfUserId(hEvent.GetInt("userid")), iHealth = GetClientHealth(GetClientOfUserId(hEvent.GetInt("userid")));
	char cHealth[32];
	...
	if (0 < iAttacker <= MaxClients && !IsFakeClient(iAttacker))
	{
		...
		if(StrEqual(StyleShowDamage[iAttacker], "NewStyle")) SendHudMessage(iAttacker, 3, -1.0, -0.6, 0x007FFFFF, 0x333333FF, 0, 0.3, 1.0, 1.0, 2.0, "%N\nУрон -%i | Осталось - %s/ХП", iVictim, iDamage, cHealth);
		else if (StrEqual(StyleShowDamage[iAttacker], "OldStyle")) PrintCenterText(iAttacker, "\nУрон -%i | Осталось - %s/ХП", iDamage, cHealth);
	}
} 
