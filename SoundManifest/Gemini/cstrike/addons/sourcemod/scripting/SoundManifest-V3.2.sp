...
void ShowMessageToAudience(SoundEventType eventType, int param1, int param2)
{
    ...
    if (g_hDebugLevel != null && g_hDebugLevel.IntValue >= 2) {
        ...
        FormatEx(sLogMessage, sizeof(sLogMessage), "%t", g_sEventMessages[eventType], param1, param2);
...
void ShowHudMessageToAudience(SoundEventType eventType, int param1, int param2)
{
    ...
    if (param1 > 0 && IsClientInGame(param1)) {
        GetClientName(param1, killerName, sizeof(killerName));
    } else {
        killerName[0] = '\0';
    }
    
    if (param2 > 0 && IsClientInGame(param2)) {
        GetClientName(param2, victimName, sizeof(victimName));
    } else {
        victimName[0] = '\0';
    }
    ...
    for (int i = 1; i <= MaxClients; i++) {
        if (IsClientInGame(i) && g_bSoundEnabled[i]) {
            char translatedMessage[256];
            FormatEx(translatedMessage, sizeof(translatedMessage), "%t", hudMessageKey, i);
            
            ReplaceString(translatedMessage, sizeof(translatedMessage), "{1}", killerName);
            ReplaceString(translatedMessage, sizeof(translatedMessage), "{2}", victimName);
